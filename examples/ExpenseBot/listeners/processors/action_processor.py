...
class ActionProcessor:
    ...
    def process_im_action(self, action):
        ...
        logging.debug(json.dumps(action, indent=4))
        # self.im_stream = self.bot_client.get_stream_client().create_im([FormParser().get_initiator_userId(action)])
        if FormParser().get_form_values(action)['action'] == 'add-expense-form':
            self.bot_client.get_message_client().send_msg(FormParser().get_stream_id(action), render_add_expense_form('listeners/expense_approval_form/html/add_expense_form.html'))

        elif FormParser().get_form_values(action)['action'] == 'add-expense-button':
            ...
            self.bot_client.get_message_client().send_msg(FormParser().get_stream_id(action), render_expense_approval_form('listeners/expense_approval_form/html/create_expense_approval_form.html'))

        elif FormParser().get_form_values(action)['action'] == 'submit-expense':
            self.employee_id = FormParser().get_initiator_userId(action)
            self.employee_name = FormParser().get_initiator_displayName(action)
            self.employee_stream = FormParser().get_stream_id(action)

            self.manager_id = FormParser().get_form_values(action)['person-selector']
            ...
            self.im_stream = self.bot_client.get_stream_client().create_im(FormParser().get_form_values(action)['person-selector'])
        ...
        elif FormParser().get_form_values(action)['action'] == 'approve-expense':
            self.manager_stream = FormParser().get_stream_id(action)
