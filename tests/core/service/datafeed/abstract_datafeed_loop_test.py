...
def fixture_bot_message_sent_event():
    ...
    return V4Event(
        type=RealTimeEvent.MESSAGESENT.name, payload=payload, initiator=initiator
    )
...
@pytest.mark.asyncio
async def test_create_listener_tasks_list_with_element(
    df_loop, listener, message_sent_v4_event
):
...
@pytest.mark.asyncio
async def test_create_listener_tasks_list_with_element_and_none(
    df_loop, listener, message_sent_v4_event
):
...
@pytest.mark.asyncio
async def test_create_listener_tasks_list_with_two_elements(
    df_loop, listener, message_sent_v4_event
):
    tasks = await df_loop._create_listener_tasks(
        [message_sent_v4_event, message_sent_v4_event]
    )
...
@pytest.mark.asyncio
async def test_create_listener_tasks_list_not_accepting_events(
    df_loop, listener, bot_message_sent_event
):
    tasks = await df_loop._create_listener_tasks([bot_message_sent_event])

    assert len(tasks) == 0
    listener.is_accepting_event.assert_called_once_with(
        bot_message_sent_event, BOT_INFO
    )


@pytest.mark.asyncio
async def test_create_listener_tasks_list(
    df_loop, listener, message_sent_v4_event, bot_message_sent_event
):
    tasks = await df_loop._create_listener_tasks(
        [message_sent_v4_event, bot_message_sent_event]
    )
...
@pytest.mark.asyncio
async def test_create_listener_tasks_list_several_listeners(
    df_loop, message_sent_v4_event, bot_message_sent_event
):
...
async def test_handle_message_sent(df_loop, listener, initiator_userid):
    ...
    listener.on_message_sent.assert_called_once_with(
        initiator_userid, payload.message_sent
    )
...
async def test_handle_shared_post(df_loop, listener, initiator_userid):
    ...
    event = V4Event(
        type=RealTimeEvent.SHAREDPOST.name, payload=payload, initiator=initiator_userid
    )
...
async def test_handle_instant_message_created(df_loop, listener, initiator_userid):
    ...
    event = V4Event(
        type=RealTimeEvent.INSTANTMESSAGECREATED.name,
        payload=payload,
    ...
    listener.on_instant_message_created.assert_called_with(
        initiator_userid, payload.instant_message_created
    )
...
async def test_handle_room_created(df_loop, listener, initiator_userid):
    ...
    event = V4Event(
        type=RealTimeEvent.ROOMCREATED.name, payload=payload, initiator=initiator_userid
    )
...
async def test_handle_room_updated(df_loop, listener, initiator_userid):
    ...
    event = V4Event(
        type=RealTimeEvent.ROOMUPDATED.name, payload=payload, initiator=initiator_userid
    )
...
async def test_handle_room_deactivated(df_loop, listener, initiator_userid):
    ...
    event = V4Event(
        type=RealTimeEvent.ROOMDEACTIVATED.name,
        payload=payload,
        initiator=initiator_userid,
    )
    ...
    listener.on_room_deactivated.assert_called_with(
        initiator_userid, payload.room_deactivated
    )
...
async def test_handle_room_reactivated(df_loop, listener, initiator_userid):
    ...
    event = V4Event(
        type=RealTimeEvent.ROOMREACTIVATED.name,
        payload=payload,
        initiator=initiator_userid,
    )
    ...
    listener.on_room_reactivated.assert_called_with(
        initiator_userid, payload.room_reactivated
    )
...
async def test_handle_user_requested_to_join_room(df_loop, listener, initiator_userid):
    ...
    event = V4Event(
        type=RealTimeEvent.USERREQUESTEDTOJOINROOM.name,
        payload=payload,
        initiator=initiator_userid,
    )
    ...
    listener.on_user_requested_to_join_room.assert_called_with(
        initiator_userid, payload.user_requested_to_join_room
    )
...
async def test_handle_user_joined_room(df_loop, listener, initiator_userid):
    ...
    event = V4Event(
        type=RealTimeEvent.USERJOINEDROOM.name,
        payload=payload,
        initiator=initiator_userid,
    )
    ...
    listener.on_user_joined_room.assert_called_with(
        initiator_userid, payload.user_joined_room
    )
...
async def test_handle_user_left_room(df_loop, listener, initiator_userid):
    ...
    event = V4Event(
        type=RealTimeEvent.USERLEFTROOM.name,
        payload=payload,
        initiator=initiator_userid,
    )
    ...
    listener.on_user_left_room.assert_called_with(
        initiator_userid, payload.user_left_room
    )


@pytest.mark.asyncio
async def test_handle_room_member_promoted_to_owner(
    df_loop, listener, initiator_userid
):
    payload = V4Payload(room_member_promoted_to_owner=V4RoomMemberPromotedToOwner())
    event = V4Event(
        type=RealTimeEvent.ROOMMEMBERPROMOTEDTOOWNER.name,
        payload=payload,
        initiator=initiator_userid,
    )
    ...
    listener.on_room_member_promoted_to_owner.assert_called_with(
        initiator_userid, payload.room_member_promoted_to_owner
    )


@pytest.mark.asyncio
async def test_handle_room_member_demoted_from_owner(
    df_loop, listener, initiator_userid
):
    payload = V4Payload(room_member_demoted_from_owner=V4RoomMemberDemotedFromOwner())
    event = V4Event(
        type=RealTimeEvent.ROOMMEMBERDEMOTEDFROMOWNER.name,
        payload=payload,
        initiator=initiator_userid,
    )
    ...
    listener.on_room_demoted_from_owner.assert_called_with(
        initiator_userid, payload.room_member_demoted_from_owner
    )
...
async def test_handle_connection_requested(df_loop, listener, initiator_userid):
    ...
    event = V4Event(
        type=RealTimeEvent.CONNECTIONREQUESTED.name,
        payload=payload,
        initiator=initiator_userid,
    )
    ...
    listener.on_connection_requested.assert_called_with(
        initiator_userid, payload.connection_requested
    )
...
async def test_handle_connection_accepted(df_loop, listener, initiator_userid):
    ...
    event = V4Event(
        type=RealTimeEvent.CONNECTIONACCEPTED.name,
        payload=payload,
        initiator=initiator_userid,
    )
    ...
    listener.on_connection_accepted.assert_called_with(
        initiator_userid, payload.connection_accepted
    )
...
async def test_handle_message_suppressed(df_loop, listener, initiator_userid):
    ...
    event = V4Event(
        type=RealTimeEvent.MESSAGESUPPRESSED.name,
        payload=payload,
        initiator=initiator_userid,
    )
    ...
    listener.on_message_suppressed.assert_called_with(
        initiator_userid, payload.message_suppressed
    )
...
async def test_handle_symphony_element(df_loop, listener, initiator_userid):
    ...
    event = V4Event(
        type=RealTimeEvent.SYMPHONYELEMENTSACTION.name,
        payload=payload,
        initiator=initiator_userid,
    )
