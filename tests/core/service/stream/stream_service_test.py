...
async def test_list_all_streams(mocked_api_client, stream_service, streams_api):
    ...
    streams_api.v1_streams_list_post.assert_called_once_with(
        filter=stream_filter, skip=0, limit=limit, session_token=SESSION_TOKEN
    )
...
async def test_add_member_to_room(stream_service, room_membership_api):
    user_id = 1234456
    room_id = "room_id"
    await stream_service.add_member_to_room(
        user_id, room_id
...
async def test_remove_member_from_room(stream_service, room_membership_api):
    user_id = 1234456
    room_id = "room_id"
    await stream_service.remove_member_from_room(
        user_id, room_id
    )  # check no exception raised
...
async def test_promote_user(stream_service, room_membership_api):
    user_id = 12345
    room_id = "room_id"
    await stream_service.promote_user_to_room_owner(
        user_id, room_id
    )  # check no exception raised
...
async def test_search_rooms(mocked_api_client, stream_service, streams_api):
    ...
    streams_api.v3_room_search_post.assert_called_once_with(
...
async def test_get_im_info(mocked_api_client, stream_service, streams_api):
    ...
    assert (
        im_details.v1_im_attributes.pinned_message_id
...
async def test_update_im(mocked_api_client, stream_service, streams_api):
    ...
    streams_api.v1_im_id_update_post.assert_called_once_with(
        id=im_id, payload=im_attributes, session_token=SESSION_TOKEN
    )
    assert (
        im_details.v1_im_attributes.pinned_message_id
        == "vd7qwNb6hLoUV0BfXXPC43___oPIvkwJbQ"
...
async def test_set_room_active_admin(mocked_api_client, stream_service, streams_api):
    ...
    streams_api.v1_admin_room_id_set_active_post.assert_called_once_with(
        id=room_id, active=active, session_token=SESSION_TOKEN
    )
    assert not room_detail.room_system_info.active
...
async def test_list_streams_admin(mocked_api_client, stream_service, streams_api):
    ...
    streams_api.v2_admin_streams_list_post.assert_called_once_with(
        filter=stream_filter, skip=skip, limit=limit, session_token=SESSION_TOKEN
    )
...
async def test_list_stream_members(mocked_api_client, stream_service, streams_api):
    ...
    streams_api.v1_admin_stream_id_membership_list_get.assert_called_once_with(
...
async def test_list_all_stream_members(mocked_api_client, stream_service, streams_api):
    ...
    streams_api.v1_admin_stream_id_membership_list_get.assert_called_once_with(
        id=stream_id, skip=0, limit=limit, session_token=SESSION_TOKEN
    )
