...
def fixture_connection_service(connection_api, auth_session):
    service = ConnectionService(connection_api, auth_session, minimal_retry_config())
    return service
...
async def test_get_connection(connection_api, connection_service):
    ...
    connection_api.v1_connection_user_user_id_info_get.assert_called_with(
        user_id="769658112378", session_token="session_token"
    )
...
async def test_list_connections(connection_api, connection_service):
    ...
    user_connections = await connection_service.list_connections(
        ConnectionStatus.ALL, [7078106126503, 7078106103809]
    )
...
async def test_create_connection(connection_api, connection_service):
    ...
    connection_api.v1_connection_create_post.return_value = deserialize_object(
        UserConnection,
        "{"
        '  "userId": 7078106126503,'
        '  "status": "PENDING_OUTGOING",'
...
async def test_accept_connection(connection_api, connection_service):
    ...
    connection_api.v1_connection_accept_post.return_value = deserialize_object(
        UserConnection,
        "{"
...
async def test_reject_connection(connection_api, connection_service):
    ...
    connection_api.v1_connection_reject_post.return_value = deserialize_object(
        UserConnection,
        "{"
