...
async def test_obo_session_username(config):
    ...
    with patch("symphony.bdk.core.auth.obo_authenticator.create_signed_jwt", return_value=signed_jwt) as create_jwt, \
        ...
        auth_api.pubkey_app_authenticate_post.assert_called_once_with(AuthenticateRequest(token=signed_jwt))
        auth_api.pubkey_app_username_username_authenticate_post.assert_called_once_with(session_token=app_token,
                                                                                        username=username)
...
async def test_obo_session_user_id(config):
    ...
    with patch("symphony.bdk.core.auth.obo_authenticator.create_signed_jwt", return_value=signed_jwt) as create_jwt, \
        ...
        create_jwt.assert_called_once_with(config.private_key, config.app_id)
        auth_api.pubkey_app_authenticate_post.assert_called_once_with(AuthenticateRequest(token=signed_jwt))
        auth_api.pubkey_app_user_user_id_authenticate_post.assert_called_once_with(session_token=app_token,
                                                                                   user_id=user_id)
...
async def test_obo_session_username_cert_authentication():
    with patch("symphony.bdk.core.auth.obo_authenticator.CertificateAuthenticationApi") as auth_api:
        ...
        auth_api.v1_app_username_username_authenticate_post.assert_called_once_with(session_token=app_token,
                                                                                    username=username)
...
async def test_obo_session_user_id_cert_authentication():
    with patch("symphony.bdk.core.auth.obo_authenticator.CertificateAuthenticationApi") as auth_api:
        ...
        assert retrieved_session_token == session_token
        auth_api.v1_app_authenticate_post.assert_called_once()
        auth_api.v1_app_user_uid_authenticate_post.assert_called_once_with(session_token=app_token, uid=user_id)
