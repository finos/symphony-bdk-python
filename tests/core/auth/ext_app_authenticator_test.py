...
async def test_authenticate_extension_app_calls_refresh():
    with patch(
        "symphony.bdk.core.auth.ext_app_authenticator.AppAuthSession.refresh"
    ) as mock_refresh:
...
async def test_authenticate_and_retrieve_tokens():
    with patch(
        ...
        assert not await extension_app_authenticator.is_token_pair_valid(
            out_app_token, "invalid symphony token"
        )
...
async def test_validate_jwt_get_pod_certificate_failure():
    with patch(
        ...
        with pytest.raises(ApiException):
            ext_app_authenticator = ExtensionAppAuthenticatorRsa(
                None, mock_pod_api, "app-id", None, minimal_retry_config()
            )
...
async def test_validate_jwt():
    with patch(
        ...
        mock_pod_api = AsyncMock()
        mock_pod_api.v1_podcert_get = AsyncMock(
            return_value=PodCertificate(certificate=certificate_content)
        )

        ext_app_authenticator = ExtensionAppAuthenticatorRsa(
            None, mock_pod_api, app_id, None, minimal_retry_config()
        )
...
async def test_cert_authenticate_and_retrieve_tokens():
    ...
    mock_authentication_api.v1_authenticate_extension_app_post.assert_called_once_with(
        auth_request=expected_request
    )
...
async def test_cert_validate_jwt():
    with patch(
        ...
        ext_app_authenticator = ExtensionAppAuthenticatorCert(
            None, mock_pod_api, app_id, minimal_retry_config()
        )
